...
export default function ViberClient(logger, bot, apiUrl, subscribedEvents) {
    this._logger = logger;
    this._bot = bot;
    this._url = apiUrl;
    this._subscribedEvents = subscribedEvents;
    this._userAgent = util.format('ViberBot-Node/%s', '2.0.0-dev');
}
...
ViberClient.prototype.sendMessage = function (
    optionalReceiver,
    messageType,
    messageData,
    ...
    if (!optionalReceiver && !optionalChatId) {
        return Promise.reject(
            new Error(`Invalid arguments passed to sendMessage. 'optionalReceiver' and 'chatId' are Missing.`)
        );
    }
    ...
    if (!messageType && !messageData && !optionalKeyboard) {
        return Promise.reject(
            new Error(
                `Invalid arguments passed to sendMessage. 'MessageData','messageType' are Missing and there's no keyboard.`
            )
        );
    }
...
ViberClient.prototype.postToPublicChat = function (senderProfile, messageType, messageData, optionalMinApiVersion) {
    ...
    if (!messageType || !messageData) {
        return Promise.reject(
            new Error(`Invalid arguments passed to postToPublicChat. 'MessageData' or 'messageType' are Missing.`)
        );
    }
    ...
    this._logger.debug(
        "Sending %s message to public chat as viber user '%s' with data",
        messageType,
        senderProfile.id,
        messageData
    );
...
ViberClient.prototype._sendRequest = function (endpoint, data) {
    ...
    return new Promise((resolve, reject) => {
        try {
            needle
                .post(url, dataWithAuthToken, options)
